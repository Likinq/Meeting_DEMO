<template>
    <el-button type="primary" @click="myList(user.data.username)">收到信息</el-button>
    <el-button type="primary" @click="myaddList(user.data.username)">已发送</el-button> 
...
<script lang="ts" setup>
    ...
    function myList(username){
        axios.get(`http://localhost:8080/message/mylist?toname=${username}`)
        .then(response => {tableData.value = response.data})
        .catch(error => {console.error("请求出错：", error)})
    }    

    function myaddList(username){
        axios.get(`http://localhost:8080/message/myaddlist?fromname=${username}`)
        .then(response => {tableData.value = response.data})
        .catch(error => {console.error("请求出错：", error)})
    }
